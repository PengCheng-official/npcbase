...
TableManager::TableManager(DataDict& dataDict, MemManager& memManager, DiskManager& diskManager)
    : dataDict_(dataDict), memManager_(memManager), diskManager_(diskManager) {}
...
RC TableManager::createTable(const char* tableName, int attrCount, const AttrInfo* attrs) {
    ...
    // 创建表并添加到数据字典
    TableId tableId;
    RC rc = dataDict_.createTable(tableName, attrCount, attrs, tableId);
    if (rc != RC_OK) {
        return rc;
    }
...
RC TableManager::dropTable(const char* tableName) {
    ...
    // 从数据字典中删除表
    return dataDict_.dropTable(tableName);
}

RC TableManager::insertRecord(const char* tableName, const char* data, int length, RID& rid) {
    ...
    RecordSlot* slot = reinterpret_cast<RecordSlot*>(frame->data + sizeof(VarPageHeader) + slotNum * sizeof(RecordSlot));
...
RC TableManager::deleteRecord(const char* tableName, const RID& rid) {
    ...
    // 标记记录为删除
    RecordSlot* slot = reinterpret_cast<RecordSlot*>(frame->data + sizeof(VarPageHeader) + rid.slotNum * sizeof(RecordSlot));
    if (slot->isDeleted) {
        memManager_.releasePage(tableInfo.tableId, rid.pageNum);
...
RC TableManager::updateRecord(const char* tableName, const RID& rid, const char* newData, int newLength) {
    if (tableName == nullptr || newData == nullptr || newLength <= 0 || newLength > MAX_RECORD_LEN) {
        return RC_INVALID_ARG;
    }

    // 先删除旧记录
    RC rc = deleteRecord(tableName, rid);
    if (rc != RC_OK) {
        return rc;
    }
    ...
    RID newRid;
    rc = insertRecord(tableName, newData, newLength, newRid);
    return rc;
}
...
RC TableManager::vacuum(const char* tableName) {
    ...
    while (currentPage != -1) {
        ...
        for (int i = 0; i < header->recordCount + header->deletedCount; i++) {
            RecordSlot* slot = reinterpret_cast<RecordSlot*>(frame->data + sizeof(VarPageHeader) + i * sizeof(RecordSlot));
